...
##### MODELS #####
# Partial pooling
summary(mod1_parPool <- glmer(correct ~ target.frames + (1 | subject), data = df, family = binomial(link=logit)))

# Complete pooling
summary(mod1_comPool <- glm(correct ~ target.frames, data = df, family = binomial(link=logit)))

df$target.frames[1:20,]
...
fun1(mod1_comPool, df$correct)
fun2(mod1_comPool, df$correct)

# Comparing my log likelihood function with the logLik function
print(comp_mods <- tibble(logLik_function=logLik(mod1_comPool),myFunction = fun2(mod1_comPool, df$correct)))
```
sandsynligheden for de værdier du har, i præcis den samme rækkefølge.
...
```{r}
print(diff <- tibble(logLikParMod = logLik(mod1_parPool), fun2(mod1_parPool, df$correct)))
```
The log-likelihood is (10622-10566) 56 off.
...
*According to an anova test the model with the lowest log likelihood ratio is model 4 (logLik(-10449).*
