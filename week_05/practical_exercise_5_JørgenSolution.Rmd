...
fun1 <- function(model, y){
  ...
  return(prod(((g.inv(fitted)^y))*(1 - g.inv(fitted))^1-df$correct))
}
```
...
```
sandsynligheden for de værdier du har, i præcis den samme rækkefølge. The likelihood functions calculates the probability for x times the probability for y again and again and again. This returns a lower and lower number, which ends out being so close to 0 as possible. This is also the reason why computers with limited precision is not preferable, since it requires great precision to arrive at the exact number.
...
```{r}
tibble(logLikParMod = logLik(mod1_parPool), fun2(mod1_parPool, df$correct))
