...
```{r}
experiment2 <- list.files(path = "/Users/WIBE/Desktop/cogSci/Methods_3/Git/emil/github_methods_3/week_03/data",     # Identify all csv files in folder
...
experiment2$correct <- (ifelse(experiment2$obj.resp == "e" & experiment2$target.type == "even", "1", 
                              ifelse(experiment2$obj.resp == "o" & experiment2$target.type == "odd", "0", NA)))
```
...
```{r}
# Describe the variables and make arguments as to which class they should belong to. Later...
# pas1: you havn't seen stimuli
# pas2: You have seen it 
```
...
```{r}
model1 <- lmer(correct ~ target.contrast, data = experiment1)
