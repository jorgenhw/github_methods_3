...
# Libraries in use
library(pacman)
p_load(tidyverse, lmer, lme4, gridExtra)
...
exp2_staircase$fitmodel1 <- fitted(model1)
...
ggplot(exp2_staircase, (aes(x = target.contrast, y = correct, color= subject)))+
  geom_point()+
  geom_line(aes(target.contrast, fitted(model1))) +
  facet_wrap(.~subject)+ 
...
nopoolfun <- function(i){
  dat <- exp2_staircase[which(exp2_staircase$subject == i),]
...
modelx <- glm(correct ~ target.contrast + subject, data = exp2_staircase, family = binomial(link = logit))

modelxx <- glm(correct ~ target.contrast*subject, data = exp2_staircase, family = binomial(link = logit))

exp2_staircase %>%
    ggplot(aes(target.contrast, as.factor(correct), color = correct)) +
    geom_point() +
    geom_line(aes(target.contrast, fitted(modelxx),
...
# ???? gotta be wrong...
ggplot(exp2_staircase)+
  geom_point(aes(x = target.contrast, y = correct, color = subject))+
...
# 
model15 <- lmer(rt.obj ~ task + (1 | subject) + (1 | trial) + (1 | task), data = exp2_experiment, REML = F)

#
model16 <- lmer(rt.obj ~ task + (1 | subject) + (1 | trial) + (1 | task), data = exp2_experiment, REML = F)
