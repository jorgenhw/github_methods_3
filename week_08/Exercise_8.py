...
plt.figure()
plt.plot(data[:, 73, :]) # Taking all repetitions (axis0), only for sensor 73
...
"""
In general participants who rated PAS2 had a slightly higher brain activity in their visual cortex(?). If this effect is significant is hard to tell, but since the lines are averages across many repetitions one could imagine that the effect is significant.
...
# calculating the average
#avr_rep_pas1 = (sen73[pas_1], axis=0)
#avr_rep_pas2 = (sen73[pas_2], axis=0)
...
y_1_2 = 2 # which is the target vector??
